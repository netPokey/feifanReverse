...
## 6. 完成度与剩余
- ✅ 已实证：架构、帧布局、53+8 ID 全分类、12 ID 逐位精确、电池包逐字节、命令字典、鉴权、167跳转表、re-sign、透传、32 发送点。
- ⬜ 剩余长尾：约 25 个 custom ID 的逐位精确公式（已有读位骨架）；0xB0 仪表打包器逐位；
  控制层每动作码→确切 CAN 帧（异步分散，可用透传/re-sign 机制替代）；idx5 容量帧 ID；gp 地址 raw32 精确化。

> 一句话：**读链路已可据此重写（解析→状态→打包→notify 全打通，12 ID 位级精确、余者有读位骨架）；
> 控链路机制完整（分发/跳转表/re-sign/透传），逐动作 CAN 帧为剩余长尾。**
