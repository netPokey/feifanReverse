...
- ✅ **控制注入真实模型**：除 BLE 透传外，控制=改写特斯拉对应控制帧的位+重签名+原 ID 回注（非构造新帧）。
- ⬜ 剩余长尾：0xB0 仪表打包器逐位；各发送点改"**哪个 CAN ID**"(接收帧追溯；改"哪些位"已全)；
  idx5 容量帧 ID；gp 地址 raw32 精确化；少数 ID 信号命名。

> 一句话：**读链路可据此重写（全 53 ID 逐位 + 打包器 + notify 全打通，16 个位级精确）；
> 控链路已揭示真实模型——"改写特斯拉控制帧的位 + 原 ID 回注"，32 个改写点的触发状态与改写位已全实证。**
