...
| 6 | `TESLA_CAN_SIGNALS.md` | 49 监控 ID 的逐信号清单 + 关键枚举值（挡位 P/R/N/D、拨杆 UP/DOWN、前备箱 MOVE/STOP…）——固件 2026.2 数据，**语义参考**（位布局仍以 #2 为准） |

### B. BLE 协议与固件结构
| # | 文档 | 作用 | 注意 |
|---|------|------|------|
| 7 | **`FIRMWARE_CH32V208_BLE_PROTOCOL_DEEP_ANALYSIS.md`** | ★BLE 命令字典 `160–240` + 三级鉴权 + 上行通知（独特，不可替代） | 现行 |
| 8 | `FIRMWARE_CH32V208_DEEP_ANALYSIS.md` | 固件整体结构 / 解码·发送例程详解 | ⚠ 部分 CAN ID 命名为**校验前旧版**，以 #4 为准 |
| 9 | `FIRMWARE_CH32V208_CAN_ID_ANALYSIS.md` | 早期 CAN ID 清单（已被 #2/#4 取代） | ⚠ 命名旧版，以 #4 为准 |
...
- **信号语义**：49 监控 ID 的逐信号清单 + 关键枚举值（挡位/拨杆/门锁/前备箱等），见 #6 `TESLA_CAN_SIGNALS.md`。
...
`fwb0`(0xB0打包器) · `symexec`(167跳转表符号执行) · `gen_signals`(生成 #6 信号清单，需 2026.2 数据)
