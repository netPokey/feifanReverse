...
仪表包 0xB0（发送封装 `0x08008f44`：`a1=0xB0; jal 0x08000c02`；数据构造 `0x08008f64`），
逐字段语义见 `TESLA_CAN_TSL_REFERENCE.md` §1.3（✔APP）。

**✔FW 0xB0 固件侧位布局**（`scripts/fwb0.py`）——打包器从 8 个 getter（RAM 状态变量读取器）取值位拼接：

| 包字节·位 | getter（状态源） | 位提取 |
|----------|-----------------|--------|
| D0[0:1] | 0x08002932 (RAM+0x68c, lhu) | `>>8 & 3` |
| D0[2:7] | 0x0800293c (RAM+0x680；读 gp+0x1f8) | `& 0x3f` |
| D1[0:4] | 0x0800291e (RAM+0x69c, lhu) | `& 0x1f` |
| D2[5:7] | 0x0800291e | `& 7 << 5` |
| D5[0:3] | 0x080048e0 (RAM+0x740, lhu) | `>>8 & 0xf` |
| D5[4:7] | 0x080049cc | `& 0xf << 4` |
| D6[6:7] | 0x08004954 | `& 3 << 6` |
| D7[7] | 0x080023b4 | `& 1 << 7` |

> getter 均为简单 RAM 读取器（`auipc+lhu/lbu` 固定地址 = 状态变量，由解析层写入）；
> 其"源 CAN ID"由本固件解码器证实（0x118 挡位 / 0x257 车速 / 0x3fe 刹车温 / 0x321 环境温等，见 §2）。
> 重写时照搬"getter RAM 偏移 → 包字节位"即可复现 0xB0。
