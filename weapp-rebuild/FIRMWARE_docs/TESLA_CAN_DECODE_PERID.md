...
## 5. STATE 型 ID 的位含义（在打包器，连接小程序）

STATE 型（0x132/0x292/0x332/0x352/0x3b3/0x3df/0x3f5/0x4e2…）解码器只 `memcpy` 整帧进 12B 状态表，
**位含义由 0xD0/0xB0 打包器决定**。已实证电池包（0xD0）来源（详见 `TESLA_CAN_PROTOCOL_FIRMWARE.md` §3.1）：

| 状态表 idx | CAN ID | 蓝牙包字段(✔APP `batteryParser`) | 原始 CAN 位 |
|-----------|--------|----------------------------------|------------|
| 1 | 0x132 BMS_hvBusStatus | 总电压=`D0\|D1<<8`×0.01V；总电流=`D2\|D3<<8`×-0.1A | D0..D3 |
| 4 | 0x332 BMS_bmbMinMax | 单体max/min=12bit×0.002V | D…(打包器待逐位) |
| 6 | 0x352 BMS_energyStatus | 出厂/当前容量×0.1/×0.02 kWh | D…(待逐位) |
| 3 | 0x292 BMS_socStatus | SOC 7/10bit | D…(待逐位) |

> 待办：反汇编打包器 `0x080097cc`(电池)/`0x08008f44`(仪表) 的 `signal_state(idx)`→读 state 偏移→写包 bit，
> 即可把 STATE 型每个 CAN ID 的原始 8 字节位含义补全（方法见 §6）。
